...
# --- 💡 高速化のためのキャッシュ機能 (10秒間はGoogleに聞きに行かない) ---
...
# --- 🍎 データ取得 ---
try:
    all_recs = get_data_cached(SHEET_NAME) # キャッシュから取得
    ...
    st.error("Googleが混雑しています。10秒待ってから再読込してください。")
    st.stop()

# --- ✍️ 書き込み関数 (ここぞという時だけ繋ぐ) ---
def write_to_google(func, *args):
    try:
        ...
        st.error("保存に失敗しました。時間をおいて試してください。")
        return False
    ...
    if st.session_state.ed["edited_rows"]:
        for r_idx, chg in st.session_state.ed["edited_rows"].items():
            if "数量" in chg:
                ...
                if write_to_google(lambda ws, r, c, v: ws.update_cell(r, c, v), int(actual_idx) + 2, 2, int(chg["数量"])):
                    st.rerun()
